...
class LeRobotLiberoSubgoalDataConfig(DataConfigFactory):
    ...
    # Optional training-only replay overlay. The directory contains one
    # ``episode_XXXXXX.png`` generated by Cosmos for each selected stage.
    generated_subgoal_dir: str | None = None
    generated_subgoal_probability: float = 1.0
    generated_subgoal_seed: int = 0
    generated_subgoal_strict: bool = False
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        if self.generated_subgoal_dir is not None:
            structure.update({"_episode_index": "episode_index", "_frame_index": "frame_index"})
        repack_inputs.append(_transforms.RepackTransform(structure))
        if self.generated_subgoal_dir is not None:
            repack_inputs.append(
                libero_subgoal_policy.GeneratedSubgoalOverlay(
                    directory=self.generated_subgoal_dir,
                    probability=self.generated_subgoal_probability,
                    seed=self.generated_subgoal_seed,
                    strict=self.generated_subgoal_strict,
                )
            )
